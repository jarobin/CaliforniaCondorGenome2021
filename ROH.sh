...
# Percentages (divide summed ROH length by 1106731338, i.e. the summed length of scaffolds 1-4, 6-32)
# BGI_N323	0.04243002
# CRW1112	0.22069630
# CYW1141	0.26048890
# VulGry1	0.05729093
...
### Verify plink ROH results with sliding window method

export SCRIPT=./SlidingWindowHet.py
cd ~/myscratch/condor
cp ~/project/condor/reference/condor/gc_PacBio_HiC/chrom_lengths.txt .
for NAME in BGI_N323 CRW1112 CYW1141 VulGry1 ; do
export NAME
qsub -N ${NAME}_winHet -t 1-32 -V -cwd -l h_rt=48:00:00,mem_free=4G -j y <<EOF
WINSIZE=1000000
STEPSIZE=100000
IDX=\$(printf %03d \${SGE_TASK_ID})
CHROM=\$(cut -f1 chrom_lengths.txt | head -n \${SGE_TASK_ID} | tail -n 1 )
VCF=\$(ls ${NAME}*_\${IDX}_*.vcf.gz )
python2.7 ${SCRIPT} \${VCF} \${WINSIZE} \${STEPSIZE} \${CHROM}
echo "SUCCESS"
EOF
done
...
for NAME in BGI_N323 CRW1112 CYW1141 VulGry1 ; do
cat ${NAME}*step.txt | \
grep -v "^chrom" | grep -v "HiC_scaffold_5" | \
awk '$5/$4 <= 5e-5 {printf "%s\t%s\t%s\n", $1, $2-1, $2+1e6-1}' | \
bedtools merge -i stdin | \
awk -v var=${NAME} '{sum+=$3-$2}END{printf "%s\t%s\n", var, sum}'
done
